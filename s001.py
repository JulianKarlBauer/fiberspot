...
region.save(
    os.path.splitext(os.path.realpath(os.path.join("output", name_image)))[0]
    + ".tiff"
)
red, green, blue = region.split()
luminance = region.copy().convert("L")

region_array = np.array(luminance)

########################################
# Plot bands
# Conclusion: Use luminance

aspect_ratio = region_array.shape[0] / region_array.shape[1]
